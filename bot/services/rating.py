...
async def get_top_infections(session: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    Returns list of {user_id, username, count}.
    """
    stmt = (
        select(
        ...
        .group_by(Infection.attacker_id, User.username)
        .order_by(desc("count"))
        .limit(limit)
    )
    ...
    rows = result.all()
    return [
        {"user_id": row.user_id, "username": row.username or str(row.user_id), "count": row.count}
        for row in rows
    ]
...
async def get_top_virus_level(session: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    Returns list of {user_id, username, virus_name, level}.
    """
    stmt = (
        select(
...
async def get_top_immunity_level(session: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    Returns list of {user_id, username, level}.
    """
    stmt = (
        select(
            User.tg_id.label("user_id"),
...
async def get_top_richest(session: AsyncSession, limit: int = 10) -> list[dict]:
    ...
    Returns list of {user_id, username, bio_coins}.
    """
    stmt = (
        select(
            User.tg_id.label("user_id"),
